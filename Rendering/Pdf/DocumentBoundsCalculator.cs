...
public static class DocumentBoundsCalculator
{
    ...
    {
        if (!TryComputeWorldBounds(document, exportZoom, linearUnit, tolerance, out var bounds))
        {
    ...
    public static bool TryComputeWorldBounds(
        CadDocument document,
        LinearDisplayUnit linearUnit,
        out Rect bounds)
        => TryComputeWorldBounds(document, exportZoom: 1.0, linearUnit, MathUtils.DefaultTolerance, out bounds);
        ...
        out Rect bounds)
    {
        ...
        bounds = ComputeContentWorldBounds(document, exportZoom, linearUnit, tolerance);
        return bounds.Width > 1e-6 && bounds.Height > 1e-6;
    }

    private static Rect ComputeContentWorldBounds(
        CadDocument document,
        double exportZoom,
        LinearDisplayUnit linearUnit,
        double tolerance)
    {
        ...
        {
            var leaderLayout = LeaderGeometry.CreateLayout(leader.Target, leader.TextPosition, safeZoom);
            var padding = Math.Max(8.0, ScreenPixelsToWorldMargin(12.0, safeZoom));
            ExpandPointWithMargin(ref hasBounds, ref minX, ref maxX, ref minY, ref maxY, leaderLayout.Target, padding);
            ExpandPointWithMargin(ref hasBounds, ref minX, ref maxX, ref minY, ref maxY, leaderLayout.Elbow, padding);
            ExpandPointWithMargin(ref hasBounds, ref minX, ref maxX, ref minY, ref maxY, leaderLayout.LandingEnd, padding);
